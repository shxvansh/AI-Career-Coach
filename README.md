...
2.  **Tool 1: `job_scraper_tool`:** The agent's first step. It uses **LangChain's WebBaseLoader** or **Tavily** to scrape the full text of the job description from the URL.
3.  **Tool 2: `resume_retriever_tool` (RAG):** The agent's second step. It queries its RAG pipeline (built from your original master resume) to get all your skills, experiences, and projects matching the job requirements.
...
5.  **Tool 3: `latex_compiler_tool`:** The agent takes the newly generated LaTeX text. This tool:
    *   Reads a `template.tex` file.
    *   Injects the new bullet points at a placeholder (e.g., `%--AGENT_BULLETS_HERE--%`).
    *   Runs a `pdflatex` command on the server to compile a new PDF.
    *   Returns the path to the `tuned_resume.pdf`.
6.  **Tool 4: `job_search_tool`:** The agent's final step. It uses **Tavily Search API** to generate smart search queries (e.g., "AI Engineer jobs with RAG and FastAPI") and retrieves 5-10 similar job URLs.
...
*   **Tools:**
    *   **Tavily API:** For robust job search and advanced scraping.
    *   **WebBaseLoader:** For basic page text extraction.
    *   **subprocess:** For executing LaTeX compilation.
